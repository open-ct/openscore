...
func (u *UnderCorrectedPaper) GetUnderCorrectedPaper(userId int64, testId int64) error {
	has, err := adapter.Where("test_id=?", testId).Where("user_id =?", userId).Get(u)
	if !has || err != nil {
...
func (u *UnderCorrectedPaper) Delete() error {
	code, err := adapter.Where(builder.Eq{"test_id": u.TestId, "user_id": u.UserId}).Delete(u)
	if code == 0 || err != nil {
...
func (u *UnderCorrectedPaper) SupervisorDelete() error {
	code, err := adapter.Where(builder.Eq{"test_id": u.TestId}).Where(" test_question_type =4 or  test_question_type =6 or  test_question_type =7").Delete(u)
	if code == 0 || err != nil {
		log.Println("delete fail")
	}
	return err
...
func (u *UnderCorrectedPaper) SelfMarkDelete() error {
	code, err := adapter.Where(builder.Eq{"test_id": u.TestId}).Where(" test_question_type =0 ").Delete(u)
	if code == 0 || err != nil {
		log.Println("delete fail")
	}
	return err
...
func (u *UnderCorrectedPaper) IsDuplicate() (bool, error) {
	...
	has, err := adapter.Where(builder.Eq{"test_id": u.TestId, "problem_type": u.ProblemType}).Get(&temp)
	if !has || err != nil {
		log.Println(err)
	}
	return has, err
...
func GetDistributedPaperByUserId(id string, up *[]UnderCorrectedPaper) error {
	err := adapter.Where("user_id = ?", id).Find(up)
	if err != nil {
...
func CountRemainingTestNumberByUserId(questionId int64, userId int64) (count int64, err error) {
	...
	count, err1 := adapter.Where("question_id = ?", questionId).Where("user_id=?", userId).Where("test_question_type=1 or test_question_type=2").Count(underCorrectedPaper)
	if err != nil {
...
func CountArbitramentUnFinishNumberByQuestionId(questionId int64) (count int64, err error) {
	underCorrectedPaper := new(UnderCorrectedPaper)
	count, err1 := adapter.Where("question_id = ?", questionId).Where("test_question_type=4").Count(underCorrectedPaper)
	if err != nil {
		log.Println("CountRemainingTestNumberByUserId err ")
	}
	return count, err1
...
func CountProblemUnFinishNumberByQuestionId(questionId int64) (count int64, err error) {
	underCorrectedPaper := new(UnderCorrectedPaper)
	count, err1 := adapter.Where("question_id = ?", questionId).Where("test_question_type=6").Count(underCorrectedPaper)
	if err != nil {
...
func CountUnScoreTestNumberByQuestionId(questionId int64) (count int64, err error) {
	underCorrectedPaper := new(UnderCorrectedPaper)
	count, err1 := adapter.Where("question_id = ?", questionId).Count(underCorrectedPaper)
	if err != nil {
...
func GetUnderCorrectedPaperByUserIdAndTestId(underCorrectedPaper *UnderCorrectedPaper, userId int64, testId int64) error {
	_, err := adapter.Where("user_id=?", userId).Where("test_id =?", testId).Where(" test_question_type !=?", 0).Get(underCorrectedPaper)
	if err != nil {
...
func GetUnderCorrectedSupervisorPaperByTestQuestionTypeAndTestId(underCorrectedPaper *UnderCorrectedPaper, testId int64) error {
	_, err := adapter.Where("test_id =?", testId).Where(" test_question_type =4 or  test_question_type =6 or  test_question_type =7").Get(underCorrectedPaper)
	if err != nil {
		log.Println("GetUnderCorrectedPaperByUserIdAndTestId err ")
	}
	return err
...
func GetSelfScorePaperByTestQuestionTypeAndTestId(underCorrectedPaper *UnderCorrectedPaper, testId int64, userId int64) error {
	_, err := adapter.Where("test_id =?", testId).Where(" test_question_type =0").Where("user_Id = ?", userId).Get(underCorrectedPaper)
	if err != nil {
...
func FindArbitramentUnderCorrectedPaperByQuestionId(arbitramentUnderCorrectedPaper *[]UnderCorrectedPaper, questionId int64) error {
	err := adapter.Where("question_id=?", questionId).Where(" test_question_type =?", 4).Find(arbitramentUnderCorrectedPaper)
	if err != nil {
...
func FindAllArbitramentUnderCorrectedPaper(arbitramentUnderCorrectedPaper *[]UnderCorrectedPaper, questionId int64) error {
	err := adapter.Where(" test_question_type =?", 4).Where("question_id= ?", questionId).Find(arbitramentUnderCorrectedPaper)
	if err != nil {
...
func FindProblemUnderCorrectedPaperByQuestionId(problemUnderCorrectedPaper *[]UnderCorrectedPaper, questionId int64) error {
	err := adapter.Where("question_id=?", questionId).Where(" test_question_type =?", 6).Find(problemUnderCorrectedPaper)
	if err != nil {
...
func FindSelfUnderCorrectedPaperByQuestionId(selfUnderCorrectedPaper *[]UnderCorrectedPaper, questionId int64) error {
	err := adapter.Where("question_id=?", questionId).Where(" test_question_type =?", 7).Find(selfUnderCorrectedPaper)
	if err != nil {
...
func FindSelfMarkPaperByQuestionId(selfMarkUnderCorrectedPaper *[]UnderCorrectedPaper, questionId int64) error {
	err := adapter.Where("question_id=?", questionId).Where(" test_question_type =?", 7).Find(selfMarkUnderCorrectedPaper)
	if err != nil {
...
func FindProblemUnderCorrectedList(problemUnderCorrectedPaper *[]UnderCorrectedPaper) error {
	err := adapter.Where(" test_question_type =?", 6).Find(problemUnderCorrectedPaper)
	if err != nil {
...
func GetDistributedTestIdPaperByUserId(id int64, up *[]int64) error {
	err := adapter.Table("under_corrected_paper").Select("test_id").Where("user_id = ?", id).Where(" test_question_type=0 or test_question_type=1 or test_question_type=2 or test_question_type=3").Find(up)
	if err != nil {
		log.Panic(err)
		log.Println("could not find any paper")
	}
...
func GetUnMarkSelfTestIdPaperByUserId(id int64, up *[]int64) error {
	err := adapter.Table("under_corrected_paper").Select("test_id").Where("user_id = ?", id).Where(" test_question_type=0 ").Find(up)
	if err != nil {
		log.Panic(err)
