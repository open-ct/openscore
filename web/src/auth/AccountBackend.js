...
export function getAccount() {
  return fetch(`${Setting.ServerUrl2}/api/get-account`, {
    method: "GET",
    credentials: "include",
  }).then(res => res.json());
}
...
export function login(code, state) {
  return fetch(`${Setting.ServerUrl2}/api/login?code=${code}&state=${state}`, {
    method: "POST",
    credentials: "include",
  }).then(res => res.json());
}
...
export function logout() {
  return fetch(`${Setting.ServerUrl2}/api/logout`, {
    method: "POST",
    credentials: "include",
  }).then(res => res.json());
}
